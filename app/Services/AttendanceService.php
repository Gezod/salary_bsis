...
class AttendanceService
{
    public static function evaluate(Attendance $a): void
    {
        ...
        // Check for half-day (only scan1 and scan2, treating scan2 as checkout)
        ...
        if ($isHalfDay) {
            // For half-day: scan1 = check-in, scan2 = check-out
            // Calculate lateness based on scan1
            ...
            // No early leave or overtime calculation for half-day
            $a->early_leave_minutes = 0;
            $a->overtime_minutes = 0;
            $a->excess_break_minutes = 0;
            $a->invalid_break = false;

            // Calculate half-day penalties
            self::computeHalfDayFine($a);
        } else {
            // Full day logic (existing)
    ...
    private static function computeHalfDayFine(Attendance $a): void
    {
        $role = strtolower($a->employee->departemen);
        ...
        // Half-day penalties (same rates as full day but only for lateness)
        $lateFine = 0;
        if ($a->late_minutes > 0) {
        ...
        // No break penalties for half-day
        $breakFine = 0;

        // Check for missing scans (only scan1 and scan2 matter for half-day)
        $absenceFine = 0;
        if (!$a->scan1) {
            $absenceFine += $rule['missing_checkin'];
        }
        if (!$a->scan2) {
            $absenceFine += $rule['missing_checkout'];
        }
        ...
        // No overtime calculation for half-day
        $a->save();
    }
